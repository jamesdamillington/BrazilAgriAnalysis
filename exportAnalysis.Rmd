...
meat_prod_Astates <- read_excel("ProductionData/Cattle_meat_production_Kg_2000_2017_all_states.xlsx", sheet = "Plan1", skip = 1)  #data for all states Astates

#dairy data are by municiaplity for all states (Amunis)
dairy_prod_Amunis <- read_excel("ProductionData/dairy_Municipalities_Brazil.xlsx", sheet = "Tabela", skip = 1, na = c("", "-", "..."))
...
```{r warning=F, message=F}
##Load exports data
meat_exp_Astates <- read_excel("ProductionData/Brazilian_Cattle_meat_exports_TOTAL_2000_2017.xls", sheet="Sheet1", range="B3:AG5", col_names=F)

dairy_exp_Astates <- read_excel("ProductionData/Brazilian_dairy_exports_TOTAL_2000_2017.xls", sheet="Sheet1", range="C3:AH5", col_names=F)
...
The mean meat export proportion of total production is `r round(mean(meat_Astates$prop),3)`

The median meat export proportion of total production is `r round(median(meat_Astates$prop),3)`

The maximum meat export proportion of total production is `r round(max(meat_Astates$prop),3)`

The mean dairy export proportion of total production is `r round(mean(dairy_Astates$prop),3)`

The median dairy export proportion of total production is `r round(median(dairy_Astates$prop),3)`

The maximum dairy export proportion of total production is `r round(max(dairy_Astates$prop),3)`
